...
à la gestion de l'affichage de chaînes de caractères de façon synchrone par le système. Nous allons également nous intéresser à la mise en place
d'un appel système (syscall). Par la suite, nous nous intéresserons à la gestion des threads utilisateur, au multithreading.
\newpage
\section{Étape 2}
...
Le fichier \textit{main.cc} est modifié pour prendre en compte l'option -sc qui permettra l'éxecution
de la console synchrone. Initialement, la création de la console était effectuée dans \textit{system.cc , fonction Initialize}, mais suite à des erreurs
...
#endif // CHANGED
\end{lstlisting}
Deplus, la fonction \textit{Cleanup() , fichier exception.cc} est modifiée, pour supprimer cette nouvelle console lors de l'arrêt de la machine :
\begin{lstlisting}[frame=single]
...
#ifdef CHANGED
    delete synchconsole;
...
PutChar:
	...
	.end PutChar
\end{lstlisting}
Le syscall \textit{PutChar} défini, il nous reste à mettre en place le handler qui se chargera de la prise en compte
...
\end{lstlisting}
\newpage
Et mettre en place les Handler associés, comme pour les précédents appels système. (fichier \textit{exception.cc, fonction ExceptionHandler}) :
\begin{lstlisting}[frame=single]
 ...
...
les registres sont initialisés. À ce niveau, le thread a le statut \textit{JUST\_CREATED}, il n'est pas encore prêt à être lancé (statut = \textit{READY}).
...
les paramètres de la fonction (pointeur vers une structure contenant les arguments).
L'espace mémoire du thread pointe vers celui du thread courant, son statut est mis à jour ( \textit{READY} ). Enfin, il est placé dans la ReadyQueue.
...
Nous réutilisons les mêmes mécaniques que précédement.

Lors de l'appel système UserThreadCreate, nous appelons la fonction do\_UserThreadCreate.
...
// Si un thread X fait un join vers notre thread, il devra attendre
// que le semaphore soit libere par notre thread lors de l'appel a do_UserThreadExit()
// pour etre debloque : on prend ce semaphore ds UserThreadJoin(int <thread cible> ).
// Ainsi, on force l'ordre d'exec, en forcant une attente du thread X sur ce semaphore.
...
\end{lstlisting}
Note : la fonction a été modifiée pour prendre en compte le multithreading au niveau user, d'où la présence de mutex ``CheckThreadExistence'',
pour assurer la cohérence de la page table (bitmap) en cas de commutation de contexte.
...
de la fonction. Nous avons créé une structure  regroupant ces paramètres. Nous passerons un pointeur vers cette structure en paramètre à StartUserThread.
...
Un nouvel appel système \textit{UserThreadExit} est créé, ce dernier fait appel à la fonction suivante (\textit{userthread.cc} ) :
